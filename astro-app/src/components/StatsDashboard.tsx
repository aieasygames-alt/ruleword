    ...
    return (
      <div class="flex items-center justify-center min-h-[400px]">
        <div class="text-center">
          ...
          <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Start Playing to Track Your Stats!</h2>
          <p class="text-gray-600 dark:text-slate-400 mb-4">Play any game and your stats will appear here.</p>
    ...
    <div class="space-y-8">
      ...
      <div class="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-2xl p-6 text-white">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-sm font-medium text-white/80">Gamer Level</div>
            <div class="text-3xl font-bold">Lv.{gamerLevel.level} — {gamerLevel.title}</div>
          </div>
        ...
        <div class="mt-4">
          <div class="flex justify-between text-sm text-white/80 mb-1">
            ...
            <span>Next: {gamerLevel.nextTitle}</span>
          </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-slate-800/50 rounded-xl p-4 border border-gray-200 dark:border-slate-700 text-center">
          ...
          <div class="text-xs text-gray-500 dark:text-slate-400">Total Plays</div>
        </div>
        <div class="bg-white dark:bg-slate-800/50 rounded-xl p-4 border border-gray-200 dark:border-slate-700 text-center">
          <div class="text-2xl mb-1">🕹️</div>
          <div class="text-2xl font-bold text-gray-900 dark:text-white">{gamesPlayed}</div>
          <div class="text-xs text-gray-500 dark:text-slate-400">Games Tried</div>
        </div>
        <div class="bg-white dark:bg-slate-800/50 rounded-xl p-4 border border-gray-200 dark:border-slate-700 text-center">
          <div class="text-2xl mb-1">⏱️</div>
          <div class="text-2xl font-bold text-gray-900 dark:text-white">{formatPlayTime(stats.totalPlayTime)}</div>
          <div class="text-xs text-gray-500 dark:text-slate-400">Play Time</div>
        </div>
        <div class="bg-white dark:bg-slate-800/50 rounded-xl p-4 border border-gray-200 dark:border-slate-700 text-center">
          <div class="text-2xl mb-1">❤️</div>
          <div class="text-2xl font-bold text-gray-900 dark:text-white">{favCount}</div>
          <div class="text-xs text-gray-500 dark:text-slate-400">Favorites</div>
        </div>
      </div>
      ...
      <div>
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="text-sm text-gray-500 dark:text-slate-400">{unlockedCount}/{achievements.length} unlocked</span>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {achievements.map(a => (
            ...
            >
              ...
              <div class="text-[10px] text-gray-500 dark:text-slate-400 mt-0.5">{a.description}</div>
            </div>
          ))}
        </div>
      </div>
            ...
            {topGames.map(([slug, p], i) => (
              <div class="flex items-center gap-3 bg-white dark:bg-slate-800/50 rounded-lg p-3 border border-gray-200 dark:border-slate-700">
                ...
                <div class="flex-1">
                  ...
                  <div class="text-xs text-gray-500 dark:text-slate-400">{p.gamesPlayed} plays</div>
                </div>
                <div class="text-right">
                  <div class="text-sm font-bold text-emerald-600 dark:text-emerald-400">{p.highScore?.toLocaleString() || '—'}</div>
                  <div class="text-xs text-gray-500 dark:text-slate-400">best score</div>
                </div>
              </div>
            ))}
          </div>
